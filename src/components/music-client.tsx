...
function MusicPage() {
    ...
    const performSearch = useCallback(async () => {
      setIsLoading(true);
      const cacheKey = `music_videos_cache`;
      ...
      if (cached) {
          try {
              ...
              console.error("Failed to parse music videos cache", e);
              localStorage.removeItem(cacheKey);
          }
      }

      try {
        const response = await getCategoryVideos("Music");
        if (response.error) {
...
export const MusicClient = withAuth(MusicPage);
